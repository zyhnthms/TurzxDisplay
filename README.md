...
+ 智谱 GLM-5.3 大模型完成**，包括：串口协议逆向、WinUI 3 界面、七种显示模式、
...
- 全部软件依赖均为 MIT，无传染性义务冲突；MIT 简短、宽松、业界接受度最高，
  最适合这种「硬件周边小工具」被自由使用、改造和分发；
- 若希望额外获得**明确的专利授权**与更完整的免责表述，可改选 **Apache-2.0**
  （与 MIT 同样宽松，多一页专利条款）；若希望强制衍生品同样开源，可选
  **GPL-3.0**，但对硬件社区的自由传播并不友好；
- 无论选择哪种，请在仓库根目录放入对应的 LICENSE 文件并在 README 保留
  第三方清单（本节）。
